...
Pozor: ve většině agregátních výpočtů vylučujeme Ministerstvo obrany (MO), protože pro něj není dostupná konzistentní časová řada. V grafech, kde jsou údaje MO zobrazeny samostatně, ze stejného důvodu není pro MO zobrazen rok 2023.
...
dta_uo_all_mean |>
  ...
  geom_point(aes(y = prumerny_plat[rok == 2023]/1e3, 
                 x = rok[rok == 2023])) +
  geom_line(size = 1, aes(y = phasal_all/1e3, colour = "Praha - celá pracovní síla")) +
  ...
  scale_colour_manual(values = c(`Praha - celá pracovní síla` = "darkred", 
                      ...
                      name = NULL) +
  labs(title = "Vývoj průměrných platů na úřadech ve srovnání s trhem práce v Praze, 2003-2023",
       subtitle = "Měsíční nominální platy/mzdy v tis. Kč")
```
...
dta_uo_all |>
  ...
  labs(title = "Průměrné platy podle úřadů, v cenách roku 2023",
       subtitle = "červeně platová úroveň Prahy\nNominální měsíční platy/mzdy v tis. Kč")
```
