...
# check if we can rebuild grouped data from chapter data for 2003-2013
zk_chap <- chapters_old %>%
  select(Year, grp, variable, value) %>%
  filter(!str_detect(variable, "AvgSal")) %>%
  filter(str_detect(variable, "skutecnost|upraveny|schvaleny")) %>%
  group_by(Year, grp, variable) %>%
  filter(!is.na(value)) %>%
  summarise(value = sum(value)) %>%
  mutate(Year = substr(Year, 1, 4))
zk_grp <- grp_old %>%
  filter(!str_detect(variable, "Minus")) %>%
  filter(str_detect(variable, "skutecnost|upraveny|schvaleny")) %>%
  mutate(Year = substr(Year, 1, 4)) %>%
  select(Year, grp, variable, value)


# not checking everything, just randomly
# can not create avgsal, probably because there are some chapters not included in the "chapter" dataset
# the mean than has less observations to work with, and produces higher mean then the group dataset
zk_chap %>% filter(grp == "PO", Year == "2010")
zk_grp %>% filter(grp == "PO", Year == "2010")

chapters_old %>%
  mutate(year = substr(Year, 1, 4)) %>%
  distinct(variable)
# now check SCHV for 2013 matches in old and new data
new <- new_dt %>% filter(rok == 2013, typ_rozpoctu == "SCHV")
old <- chapters_old %>%
  mutate(year = substr(Year, 1, 4)) %>%
  filter(year == 2013 & str_detect(variable, "schvaleny")) %>%
  select("rok" = year, "name" = grp, "kap_num" = KapNum, variable, value) %>%
  reshape(timevar = c("variable"), idvar = c("rok", "name", "kap_num"), direction = "wide") %>%
  rename(
    "oppp" = value.OPPP_schvaleny,
    "prostredky_na_platy" = value.Platy_schvaleny,
    "prostredky_na_platy_a_oppp" = value.PlatyOPPP_schvaleny,
    "prumerny_plat" = value.AvgSal_schvaleny,
    "pocet_zamestnancu" = value.Zam_schvaleny
  ) %>%
  ...
  filter(!is.na(kap_num), !is.na(name), prostredky_na_platy_a_oppp > 0)


new <- new %>%
  select(name, rok, kap_num, prostredky_na_platy_a_oppp, pocet_zamestnancu, prumerny_plat) %>%
  as_tibble()
old <- old %>%
  select(name, rok, kap_num, prostredky_na_platy_a_oppp, pocet_zamestnancu, prumerny_plat) %>%
  as_tibble()
# checking all groups and kap_nums - everything is correct, sow e have correct data
for (i in 1:nrow(new)) {
  print(i)
  cur_kap <- new$kap_num[i]
  cur_name <- new$name[i]
  old_row <- old %>% filter(name == cur_name, kap_num == cur_kap)
  if (old_row$prostredky_na_platy_a_oppp * 1000 != new$prostredky_na_platy_a_oppp[i] | old_row$pocet_zamestnancu != new$pocet_zamestnancu[i] | round(old_row$prumerny_plat, 0) != new$prumerny_plat[i]) {
    print(old_row)
    print(new[i, ])
  }
}
...
# some differences in the kap nums
new_kaps <- new %>%
  distinct(kap_num) %>%
  arrange(kap_num) %>%
  pull()
old_kaps <- old %>%
  ...
  arrange(kap_num) %>%
  pull()
setdiff(new_kaps, old_kaps)
setdiff(old_kaps, new_kaps) # these were disbanned by the state, so it is okay



# check 2013-2020 from monitor
validity_check_monitor <- function(y, typ_vydaju = "platy") {
  ...
  new_dt_colname <- case_when(
    typ_vydaju == "platy" ~ "prostredky_na_platy",
    typ_vydaju == "oppp" ~ "oppp",
    typ_vydaju == "platy_oppp" ~ "prostredky_na_platy_a_oppp"
  )
  polozka_vector <- case_when(
    typ_vydaju == "platy" ~ c("501"),
  ...
  for (k in KAPS) {
    ...
    subset_new <- new_dt %>%
      filter(name == "OSS", kap_num == k, rok == y) %>%
      select(typ_rozpoctu, new_dt_colname)
    ...
    schv_new <- subset_new %>%
      ...
      select(new_dt_colname) %>%
      pull()
    uprav_new <- subset_new %>%
      filter(typ_rozpoctu == "UPRAV") %>%
      select(new_dt_colname) %>%
      pull()
    skut_new <- subset_new %>%
    ...
    if (subset_monitor["budget_amended"] != uprav_new) {
      print(paste(
        "Year:", y, "Kapitola:", k, "Budget: UPRAV",
...
# check 2013-2020 with previous data
...
dt_cost <- new_dt %>%
  ...
  rename(
    "rozp" = prostredky_na_platy_a_oppp.SCHV,
...
new_dt_reshaped <- rbind(dt_cost, dt_count) %>%
  ...
  # other years not included in previous data
  as_tibble() %>%
  filter(type != "ROPO") %>%
  # not in our the old dataset, but computable from OSS and PO
...
prev_dt_match <- prev_dt %>%
  ...
  # comments and totals not included
  filter(type != "Príslušníci a vojáci") %>%
  # not in the main dataset
  mutate(
    kap_num = as.integer(kap_num),
