...
cols <- RColorBrewer::brewer.pal(length(unique(dt$kategorie_2014_cz))-1, name = "Set3")

df_col<-cbind("kategorie_2014_cz" = unique(dt$kategorie_2014_cz),cols) %>% as.data.frame()
dt<-left_join(dt,df_col,by="kategorie_2014_cz")
dt$cols <- factor(dt$kategorie_2014_cz, labels = cols)
```
...
* Větší část těchto státních zaměstnanců spadá pod příspěvkové organizace (265 tisíc), zahrnující především školy, a menší část ostatních státních zaměstnanců (198 tisíc) spadá pod řadu různorodých organizací, od armády a policii až po ministerstva a úřady práce.
* Jen malou část státních zaměstnanců lze označit za státní úředníky (zhruba 78 tisíc).
* Počet státních úředníků v průběhu posledních dvou dekád kolísal. Mezi roky 2003 a 2010 jejich počet mírně narostl o necelé dva tisíce. Mezi roky 2010 a 2011 došlo k poklesu o tři tisíce během ekonomické krize v době Nečasovy vlády. Mezi lety 2010 a 2015 došlo k celkovému nárůst o pět tisíc státních úředníků a do roku 2020 jejich počet nedoznal významnější změny.
* Ačkoliv reálné průměrné platy státních úředníků od roku 2013 soustavně rostly, od roku 2018 zaostával tento růst znatelně za nárůstem mezd v celé ekonomice.
...
Proto v druhé části této studie analyzujeme vývoj jejich počtů a platů mezi roky 2003 a 2020.
...
Pro zodpovězení otázek v této studii využíváme odpovídající metodologický přístup (analýza dat) a nejvhodnější dostupná data. Konkrétně se jedná o stejný datový zdroj (detailní **data od Ministerstva financí ze Státního závěrečného účtu**) jako ve studii IDEA vydané v červnu 2014. Data z této dřívější [studie](https://idea.cerge-ei.cz/studies/2014-06-statni-urednici-kolik-jich-vlastne-je-kde-a-za-jake-platy-pracuji) pro roky 2003 až 2012 rozšiřujeme o data pro roky 2013 až 2020, která jsme obdrželi od Ministerstva financí 10. 3. 2021. Metodologické a datové detaily popisujeme v separátní sekci v příloze.

## Státní zaměstnanci
### Počet státních zaměstnanců

Celkem 464 tisíc zaměstnanců je placeno ze státního rozpočtu dle údajů pro rok 2020. Tyto státní zaměstnance můžeme dále členit dle regulace zaměstnanosti (graf 1) nebo dle rozpočtových kapitol (graf 2).^[Všechny grafy byly vytvořeny pomocí plotly. Pro detailnější analýzu se může hodit udělat drag+drop výseč přímo v grafu (Zoom button), nebo odškrtnout některé z kategorií v legendě napravo. Double-clickem na kategorii v legendě je možné skrýt všechny kategorie až na tu vybranou kategorii. Pro reset je tam “Reset axes” button.] V příloze vysvětlujeme základní logiku těchto dvou členění, které při diskuzi výsledků níže kombinujeme.
...
plot_ly(bar_dt,
  x = ~kap_name, y = ~ pocet_zamestnancu / 1000,color=~kategorie_2014_cz,type="bar",
  ...
  layout(
    ...
    xaxis = list(title = "Kapitoly",tickmode = "array",categoryorder = "array", categoryarray = arrange(bar_dt, desc(pocet_zamestnancu_agg))$kap_name),
...
A2<-plot_ly(bar_dt,
  ...
  layout(
    ...
    xaxis = list(title = "Kapitoly", categoryorder = "array", categoryarray = arrange(bar_dt, desc(prostredky_na_platy_agg))$kap_name),
...
bar_dt%>%filter(prumerny_plat>25000) %>%
plot_ly(
  ...
  hovertemplate = ~ ifelse(prostredky_na_platy > 0, paste("<extra></extra>", " Zdroj:", name, "<br>", "Kapitola:", cz_kap_name, "<br>", "Pr\u016Fm\u011Brn\u00FD plat:", format(prumerny_plat, big.mark = " "), "K\u010D"), ""),
  hoverinfo = "text"
) %>%
  layout(
    hovermode = "closest",
...
aux<-bar_dt %>% 
  filter(name %in% c("UO","OSS_SS")) %>% ungroup()
B3<-aux %>%
  group_by(kategorie_2014_cz) %>%
  ...
  group_map(~ plot_ly(
    ...
    layout(
      ...
      title = list(text = "Graf B3. Pr\u016Fm\u011Brn\u00E9 platy st\u00E1tn\u00EDch zam\u011Bstnanc\u016F (v tis\u00EDc\u00EDch)", x = 50, y = 0.98), legend = list(x = 50, y = 0.5), showlegend = F
    ), keep = TRUE) %>%
...
A8<-dt %>%
  ...
  layout(
    ...
    annotations = list(text = str_wrap("Pro přehlednost vynechány kapitoly MZV a MV, které by vzhledem ke strukturálním změnám způsobily kumulativní nárůst o zhruba 60% na Ministerstvech mezi roky 2011-2012.",40),
        ...
        showarrow = FALSE),
    showlegend = FALSE,
    xaxis = list(title = "Zm\u011Bna pr\u016Fm\u011Brn\u00E9ho platu", ticksuffix = "%",range = c(-5,65)),
...
zam_2019<-dt %>% filter(rok == 2018,typ_rozpoctu == "SKUT") %>%
 ...
 zk <-dt %>% filter(kap_name == "MV",typ_rozpoctu == "SKUT",name=="UO") %>% select(typ_rozpoctu,rok,pocet_zamestnancu) %>%
