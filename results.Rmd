...
grdclr <- "grey25"
cap_col <- "mediumblue"
mrg2 <- list(t = 50, autoexpand = TRUE)
```
...
graf_A1 <- tree_data %>%
  ...
  layout(title = list(font=list(color = cap_col),text = "<b>Graf A1. Výdaje na platy státních zaměstnanců dle regulace zaměstnanosti, rok 2020</b>", 
                      y = 0.98)) %>%
...
graf_1 <- tree_data %>%
  ...
  layout(title = list(font=list(color = cap_col),
                      text = "<b>Graf 1. Počet státních zaměstnanců dle regulace zaměstnanosti, rok 2020</b>", 
                      y = 0.98)) %>%
...
graf_2 <- bar_dt %>% group_by(kategorie_2014_cz)%>%
  ...
  layout(
    ...
    title = list(font=list(color = cap_col),
                 text = "<b>Graf 2. Po\u010Det st\u00E1tn\u00EDch zam\u011Bstnanc\u016F dle rozpo\u010Dtov\u00FDch kapitol (rok 2020, tis\u00EDce)</b>", y = 0.97),
    ...
    yaxis = list(gridcolor = grdclr,title = "Po\u010Det zam\u011Bstnanc\u016F"), barmode = "stack",
    margin = mrg2
  ) %>%
...
graf_A2 <- plot_ly(bar_dt,
  ...
  layout(
    ...
    hovermode = "closest",
    title = list(font=list(color = cap_col),
                 text = "<b>Graf A2. Výdaje na platy státních zaměstnanců (rok 
                 2020, miliardy K\u010D)</b>", y = 0.98),
    xaxis = list(title = "Kapitoly", categoryorder = "array", 
                 categoryarray = arrange(bar_dt, desc(prostredky_na_platy_agg))$kap_name),
    yaxis = list(title = "Platy",gridcolor = grdclr),
...
graf_3 <- bar_dt %>% 
  ...
  plot_ly(
    ...
    type = "scatter" , mode = "line+markers",line = list(color='#999999',width=3),
    hovertemplate = ~ ifelse(prostredky_na_platy > 0, 
  ...
  layout(
    ...
    hovermode = "closest",
    title = list(font=list(color = cap_col),text = "<b>Graf 3. Pr\u016Fm\u011Brn\u00E9 platy st\u00E1tn\u00EDch zam\u011Bstnanc\u016F (rok 2020, tis\u00EDce K\u010D)</b>", y = 1.1),
    xaxis = list(title = "",gridcolor = "white"),
    yaxis = list(title = "",gridcolor = "white"),
    showlegend = F, margin = mrg2
  ) %>%
  config(modeBarButtonsToRemove = btnrm, displaylogo = FALSE)
graf_3
```
...
graf_B3 <- aux %>%
  ...
  group_map(~ plot_ly(
    ...
    layout(
      plot_bgcolor='whitesmoke',
      yaxis = list(title = "", range = c(20, 60), gridcolor = grdclr), 
      xaxis = list(tickfont = list(size = 10), 
                   showticklabels = T, 
                   tickangle = 0, 
                   "categoryorder" = "total descending"),
      title = list(font=list(color = cap_col),
                   text = "<b>Graf B3. Pr\u016Fm\u011Brn\u00E9 platy st\u00E1tn\u00EDch zam\u011Bstnanc\u016F (tis\u00EDce K\u010D)</b>", 
...
graf_A3 <- bar_dt %>%
  ...
  group_map(~ plot_ly(
    ...
    layout(margin = list(t = 50),
      plot_bgcolor='whitesmoke',
      yaxis = list(title = "",range=c(-50,70),gridcolor = grdclr,
                   ticksuffix="%"), xaxis = list(tickfont = list(size = 10), 
                                                 showticklabels = T, 
                                                 tickangle = 90, 
                                                 "categoryorder" = "total ascending"),
      title = list(font=list(color = cap_col),
                   text = str_wrap("<b>Graf A3. Pr\u016Fm\u011Brn\u00E9 platy st\u00E1tn\u00EDch zam\u011Bstnanc\u016F ve vztahu k pr\u016Fm\u011Brn\u00E9 mzd\u011B v n\u00E1rodn\u00EDm hospod\u00E1\u0159stv\u00ED, rok 2020</b>",70), 
...
graf_4 <- dt %>%
  ...
  layout(
    plot_bgcolor='whitesmoke',
    title = list(font=list(color = cap_col),text = "<b>Graf 4. Počet státních úředníků 2003–2020 (tisíce)</b>", 
                 y = 0.98),
    annotations = list(text = str_wrap("Pro srovnatelnost v čase graf nezahrnuje zaměstnance ministerstev vnitra a zahraničních věcí, viz <b> Přílohu 1: Data a metodologie. </b> Graf 4 s kapitolami MZV a MV je v příloze.",40),
                       align='left',
                       xref='paper',
                       yref='paper',
                       x=1.33,
    ...
    barmode = "stack", legend = list(x = 100, y = 0.5), margin = mrg2
  ) %>%
  config(modeBarButtonsToRemove = btnrm, displaylogo = FALSE)
...
graf_A4 <- vyvoj_bar %>%
  ...
  layout(
    plot_bgcolor='whitesmoke',
    title = list(font=list(color = cap_col),
                 text = "<b>Graf A4. Výdaje na platy státních úředníků 2003–2020 (mld. Kč)</b>", 
                 y = 0.98),
    annotations = list(text = str_wrap("Pro srovnatelnost v čase graf nezahrnuje zaměstnance ministerstev vnitra a zahraničních věcí, viz <b> Přílohu 1: Data a metodologie. </b>",40),
                       align='left',
                       xref='paper',
                       yref='paper',
                       x=1.33,
                       ...
                       showarrow = FALSE),
    xaxis = list(title = ""),
    yaxis = list(title = "", gridcolor = grdclr),
    barmode = "stack", legend = list(x = 100, y = 0.5), margin = mrg2
  ) %>%
  config(modeBarButtonsToRemove = btnrm, displaylogo = FALSE)
...
graf_A5 <- vyvoj_bar %>%
  ...
  layout(margin = mrg2,
    plot_bgcolor='whitesmoke',
    title = list(font=list(color = cap_col),
                 text = "<b>Graf A5. Reálné výdaje na platy státních úředníků 2003–2020 (mld. Kč)</b>", 
                 y = 0.98),
    annotations = list(text = str_wrap("Pro srovnatelnost v čase graf nezahrnuje zaměstnance ministerstev vnitra a zahraničních věcí, viz <b> Přílohu 1: Data a metodologie. </b>",40),
                       align='left',
                       xref='paper',
                       ...
                       showarrow = FALSE),
    xaxis = list(title = ""),
    yaxis = list(title = "Reálné výdaje na platy (mld. Kč, ceny roku 2020)",gridcolor = grdclr),
    barmode = "stack", legend = list(x = 100, y = 0.5), margin = mrg2
  ) %>%
  config(modeBarButtonsToRemove = btnrm, displaylogo = FALSE)
```
...
graf_A6 <- aux2 %>%
  ...
  layout(margin = mrg2,
    plot_bgcolor='whitesmoke',
    title = list(font=list(color = cap_col),text = str_wrap("<b>Graf A6. Změna reálných výdajů na platy státních úředníků od roku 2020</b>",70), y = 0.98),
    annotations = list(text = str_wrap("Pro srovnatelnost v čase graf nezahrnuje zaměstnance ministerstev vnitra a zahraničních věcí, viz <b> Přílohu 1: Data a metodologie. </b>",40),
                       align='left',
                       xref='paper',
                       yref='paper',
                       ...
                       showarrow = FALSE),
    xaxis = list(title = ""),
    yaxis = list(title = "Změna reálných výdajů na platy oproti roku 2003 <br />(ceny roku 2020)", 
                 tickprefix = "+", ticksuffix = "%",gridcolor = grdclr), 
...
graf_5 <- dt %>%
  ...
  layout(
    plot_bgcolor='whitesmoke',
    title = 
      list(font=list(color = cap_col),
           text = "<b>Graf 5: Průměrné platy státních úředníků 2003–2020 (tisíce Kč)</b>", 
           y =0.98),
    xaxis = list(title = ""),
    yaxis = list(title = "Reálné průměrné mzdy (tis. Kč, ceny roku 2020)",
                 gridcolor = grdclr), legend = list(x = 100, y = 0.5),
    margin = mrg2
  ) %>%
  config(modeBarButtonsToRemove = btnrm, displaylogo = FALSE)
...
graf_A7 <- dt %>%
  ...
  layout(
    plot_bgcolor='whitesmoke',
    title = list(font=list(color = cap_col),
                 text = "<b>Graf A7. Změna reálných průměrných platů státních úředníků</b>", 
                 y = 0.98),
    annotations = list(text = str_wrap("Pro srovnatelnost v čase graf nezahrnuje zaměstnance ministerstev vnitra a zahraničních věcí, viz <b>Přílohu 1: Data a metodologie. </b>",40),
                       align='left',
                       xref='paper',
                       ...
                       showarrow = FALSE),
    xaxis = list(title = ""),
    yaxis = list(title = "Změna reálného průměrného platu oproti roku 2003<br />(ceny roku 2020)", 
                 tickprefix = "+", ticksuffix = " %",gridcolor = grdclr),
    legend = list(x = 100, y = 0.5), margin = mrg2
  ) %>%
  config(modeBarButtonsToRemove = btnrm, displaylogo = FALSE)
...
graf_6 <- dt %>%
  ...
  layout(plot_bgcolor='whitesmoke',
    title = list(font=list(color = cap_col),
                 text = str_wrap("<b>Graf 6. Průměrný plat ve státní správě ve vztahu k průměrné mzdě v národním hospodářství</b>",60), 
                 y = 0.95),
    annotations = list(text = str_wrap("Pro ministerstva a ostatní ústřední orgány použité hodnoty průměrné mzdy v Praze. V ostatních případech je jako reference použitý průměrný plat v národním hospodářství. Hodnota 100% znamená, že průměrný plat v kategorii je stejný jako průměrný plat v národním hospodářství. Popisky “Největší a nejmenší nárust” zde vyjadřují procentuální změnu platu ve vztahu k průměrnému platu v ČR. Tedy pokud plat na Ministerstvu financí byl v jednom roce roven 115% pražského platu, a další rok vzrostl na 130%, největší nárůst by byl 15%.",40),
                       ...
                       showarrow = FALSE),
    xaxis = list(title = ""),
    yaxis = list(title = "", ticksuffix = "%",gridcolor = grdclr), legend = list(x = 100, y =0.8),
    margin = mrg2
  ) %>%
  config(modeBarButtonsToRemove = btnrm, displaylogo = FALSE)
...
graf_A8 <- dt %>%
  ...
  layout(
    ...
    yaxis = list(title = "Zm\u011Bna po\u010Dtu zam\u011Bstnanc\u016F", 
                 ...
                 tickprefix = "+", 
                 ticksuffix = "%", showticksuffix = "all",gridcolor = grdclr), 
    legend = list(x = 100, y = 0.5), margin = mrg2
  ) %>%
  add_markers() %>%
...
graf_A9 <- dt %>%
  ...
  group_map(~ plot_ly(
    ...
    layout(plot_bgcolor='whitesmoke',
           xaxis = list(title= ""),
           yaxis = list(title = "", 
                        range = c(-30, 10), ticksuffix = " %",
                        gridcolor = grdclr), 
           xaxis = list(tickangle = 90), legend = list(x = 100, y = 0.5)), 
  keep = TRUE) %>%
  subplot(nrows = 5, shareX = TRUE, shareY = TRUE, titleY = FALSE) %>%
  layout(title = list(font=list(color = cap_col),
                      text = "<b>Graf A9. Rozdíl mezi schváleným a skutečným počtem zaměstnanců</b>", 
                      y = 0.98),
         showlegend = FALSE, 
         annotations = list(x = 0 , y = 0.5, text = "Záporné = méně skutečných než schválených",
                            font = list(size = 14),
                            xshift = -50, textangle = 270, showarrow = F, 
                            xref='paper', yref='paper'),
         margin = mrg2) |> 
  config(modeBarButtonsToRemove = btnrm, displaylogo = FALSE)
```

```{r platy_skut_rozp_kap}
...
graf_A10 <- dt %>%
  ...
  layout(title = list(font=list(color = cap_col),
                      text = "<b>Graf A10. Rozdíl v průměrných platech mezi schváleným rozpočtem a skutečností</b>",
                      y = 0.98), showlegend = FALSE, margin = mrg2,
         annotations = list(x = 0 , y = 0.5, text = "Kladné = skutečný průměrný plat vyšší než schválený",
                            font = list(size = 14),
                            xshift = -50, textangle = 270, showarrow = F, 
                            xref='paper', yref='paper')) |> 
  config(modeBarButtonsToRemove = btnrm, displaylogo = FALSE)
...
graf_A11 <- dt %>%
  ...
  group_map(~ plot_ly(
    ...
    layout(plot_bgcolor='whitesmoke',
           xaxis = list(title=""),
           yaxis = list(title = "Záporné = méně skutečných než schválených", 
                        range = c(-15, 3), ticksuffix = " %",
                        gridcolor = grdclr), 
           legend = list(x = 100, y = 0.5), showlegend = F), keep = TRUE) %>%
  subplot(nrows = 2, shareX = TRUE, shareY = TRUE, titleY = FALSE) %>%
  layout(title = list(font=list(color = cap_col),
                      text = "<b>Graf A11. Rozdíl mezi schváleným a skutečným počtem zaměstnanců</b>", 
                      y = 0.98), margin = mrg2,
         annotations = list(x = 0 , y = 0.5, text = "Záporné = skutečný počet nižší než schválený",
                            font = list(size = 14),
                            xshift = -50, textangle = 270, showarrow = F, 
                            xref='paper', yref='paper')) |> 
  config(modeBarButtonsToRemove = btnrm, displaylogo = FALSE)
...
graf_A12 <- dt %>%
  ...
  group_map(~ plot_ly(
    ...
    layout(plot_bgcolor='whitesmoke',
           xaxis = list(title=""),
           yaxis = list(title = "", range = c(0, 16), ticksuffix = "%",gridcolor = grdclr), 
           legend = list(x = 100, y = 0.5), showlegend = F), keep = TRUE) %>%
  subplot(nrows = 2, shareX = TRUE, shareY = TRUE) %>%
  layout(title = list(font = list(color = cap_col),
                      text = "<b>Graf A12. Rozdíl v průměrných platech mezi schváleným rozpočtem a skutečností </b>", 
                      y = 0.98), margin = mrg2,
         annotations = list(x = 0 , y = 0.5, text = "Kladné = skutečný průměrný plat vyšší než schválený",
                            font = list(size = 14),
                            xshift = -50, textangle = 270, showarrow = F, 
...
graf_A13 <- dt %>%
  ...
  group_map(~ plot_ly(
    ...
    layout(plot_bgcolor='whitesmoke',
           yaxis = list(title = "Nárůst průměrného platu od roku 2003",
                        range = c(15, 75), ticksuffix = "%",gridcolor = grdclr),
  ...
  subplot(nrows = 2, shareX = T, shareY = T, titleY = F, titleX = F) %>%
  layout(title = list(font=list(color = cap_col),text = "<b>Graf A13. Nárůst průměrných platů od roku 2003</b>",
                      xaxis = list(title = ""), y = 0.98), margin = mrg2,
         annotations = list(list(x = 0 , y = 0.5, text = "Nárůst průměrného platu od roku 2003",
                                 font = list(size = 14),
                                 xshift = -50, textangle = 270, showarrow = F, 
                                 xref='paper', yref='paper'),
                            list(y = 0 , x = 0.5, text = "Průměrný plat v roce 2003 (tisíce Kč)",
                                 font = list(size = 14),
                                 yshift = -40,
...
graf_A0 <- plot_ly(pubsec, type = "scatter", mode = "lines+markers", line = list(width = 4),
    ...
    layout(plot_bgcolor='whitesmoke',
           yaxis = list(title = NA, range = c(0, 1800),gridcolor = grdclr),
           xaxis = list(title = NA),
           legend = list(x = 100, y = 0.5), showlegend = F,
           title = list(font=list(color = cap_col),
                        text = "<b>Graf A0. Počet zaměstnanců veřejného sektoru (tis. přepočtených osob)</b>",
                        y = 0.98), margin = mrg2) %>%
  config(modeBarButtonsToRemove = btnrm, displaylogo = FALSE)
...
graf_B4 <- dt %>%
  ...
  layout(
    plot_bgcolor='whitesmoke',
    title = list(font=list(color = cap_col),text = "<b>Graf B4. Počet státních úředníků 2003–2020 (v tisících)</b>", y = 0.98),
