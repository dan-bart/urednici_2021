...
kat_order_all <- c("Ministerstva", "Ostatní ústřední", "Neústřední st. správa","Státní úředníci celkem", 
...
color_map <- c("Ministerstva"=col2hex("darkgrey"), 
               "Neústřední st. správa"=col2hex("cornflowerblue"),
               "Ostatní ústřední"=col2hex("firebrick"),
               "Ostatní vč. armády"=col2hex("darkseagreen"), 
               "Příspěvkové organizace"=col2hex("lightpink"),
               "Sbory"=col2hex("royalblue4"), 
               "Ústřední orgány" = col2hex("indianred"), 
               "Státní správa" = col2hex("lightskyblue"), 
               "Organizační složky státu" = col2hex("lightcyan"),
                 "Státní úředníci celkem" = col2hex("yellow"))
...
dt <- dt %>%
  mutate(kategorie_2014_cz = ifelse(kategorie_2014_cz=="Státní správa","Státní úředníci celkem",kategorie_2014_cz)) %>%
...
graf_A1 <- tree_data %>%
  ...
  layout(title = list(font=list(color = "mediumblue"),text = "<b>Graf A1. Výdaje na platy státních zaměstnanců dle regulace zaměstnanosti</b>", 
                      y = 0.98)) %>%
...
graf_1 <- tree_data %>%
  ...
  layout(title = list(font=list(color = "mediumblue"),text = "<b>Graf 1. Počet státních zaměstnanců dle regulace zaměstnanosti</b>", 
                      y = 0.98)) %>%
...
graf_2<-bar_dt %>% group_by(kategorie_2014_cz)%>%
  ...
  layout(
    ...
    annotations = list(text = str_wrap("MŠMT zkresluje graf vzhledem k zahrnutí  učitelů v kategorii “Příspěvkové organizace”. Lze odflitrovat v legendě nebo v grafu.",40),
                       align='left',
                       xref='paper',
                       yref='paper',
                       x=1.33,
                       ...
                       borderwidth=1,
                       showarrow = FALSE),
    title = list(font=list(color = "mediumblue"),text = "<b>Graf 2. Po\u010Det st\u00E1tn\u00EDch zam\u011Bstnanc\u016F dle rozpo\u010Dtov\u00FDch kapitol (tis\u00EDce)</b>", y = 0.98),
    ...
    yaxis = list(gridcolor="darkgrey",title = "Po\u010Det zam\u011Bstnanc\u016F"), barmode = "stack"
  ) %>%
...
graf_A2 <- plot_ly(bar_dt,
              ...
              hovertemplate = ~ ifelse(prostredky_na_platy > 0, paste("<extra></extra>", " Zdroj:", 
                                                                      name, "<br>", "Kapitola:", 
                                                                      cz_kap_name, "<br>", "Platy celkem:", format(prostredky_na_platy, big.mark = " "), "K\u010D", "<br>", "Celkem za kapitolu: ",
  ...
  layout(
    plot_bgcolor='whitesmoke',
    hovermode = "closest",
    title = list(font=list(color = "mediumblue"),text = "<b>Graf A2. Výdaje na platy státních zaměstnanců (miliardy K\u010D)</b>", y = 0.98),
    xaxis = list(title = "Kapitoly", categoryorder = "array", 
                 categoryarray = arrange(bar_dt, desc(prostredky_na_platy_agg))$kap_name),
    yaxis = list(title = "Platy",gridcolor = "darkgrey"),
...
graf_3<-bar_dt %>% 
  ...
  layout(
    ...
    hovermode = "closest",
    title = list(font=list(color = "mediumblue"),text = "<b>Graf 3. Pr\u016Fm\u011Brn\u00E9 platy st\u00E1tn\u00EDch zam\u011Bstnanc\u016F (tis\u00EDce K\u010D)</b>", y = 0.98),
    xaxis = list(title = "",gridcolor = "darkgrey"),
    yaxis = list(title = "",gridcolor = "darkgrey"),
    showlegend = F
  ) %>%
  config(modeBarButtonsToRemove = c("zoomIn2d", "zoomOut2d", "pan2d", "lasso2d", 
                                    "select2d", "autoScale2d"), 
...
graf_B3 <- aux %>%
  ...
  group_map(~ plot_ly(
    ...
    hovertemplate = ~ ifelse(prumerny_plat > 0, paste(
      ...
      cz_kap_name, "<br>", "Pr\u016Fm\u011Brngraf_B3\u00FD plat:",
      format(prumerny_plat, big.mark = " "), "K\u010D"
    ), ""), 
    hoverinfo = "text"
  ) %>%
    ...
    layout(
      plot_bgcolor='whitesmoke',
      yaxis = list(title = "", range = c(20, 60),gridcolor = "darkgrey"), 
      xaxis = list(tickfont = list(size = 10), 
                   showticklabels = T, 
                   tickangle = 0, 
                   "categoryorder" = "total descending"),
      title = list(font=list(color = "mediumblue"),text = "<b>Graf B3. Pr\u016Fm\u011Brn\u00E9 platy st\u00E1tn\u00EDch zam\u011Bstnanc\u016F (tis\u00EDce K\u010D)</b>", x = 50, y = 0.98), legend = list(x = 50, y = 0.5), 
      showlegend = F
    ), keep = TRUE) %>%
...
graf_A3 <- bar_dt %>%
  ...
  group_map(~ plot_ly(
    ...
    layout(margin = list(t = 50),
      plot_bgcolor='whitesmoke',
      yaxis = list(title = "",range=c(-50,70),gridcolor = "darkgrey",
                   ticksuffix="%"), xaxis = list(tickfont = list(size = 10), 
                                                 showticklabels = T, 
                                                 tickangle = 90, 
                                                 "categoryorder" = "total ascending"),
      title = list(font=list(color = "mediumblue"),text = str_wrap("<b>Graf A3. Pr\u016Fm\u011Brn\u00E9 platy st\u00E1tn\u00EDch zam\u011Bstnanc\u016F ve vztahu k pr\u016Fm\u011Brn\u00E9 mzd\u011B v n\u00E1rodn\u00EDm hospod\u00E1\u0159stv\u00ED</b>",70), 
                   x = 50, y = 0.95), legend = list(x = 50, y = 0.5), 
      showlegend = F
    ), keep = TRUE) %>%
  subplot(nrows = 2, shareY = F,margin = 0.05)
```
...
graf_4<-dt %>%
  ...
  layout(
    plot_bgcolor='whitesmoke',
    title = list(font=list(color = "mediumblue"),text = "<b>Graf 4. Počet státních úředníků 2003-2020 (tisíce)</b>", 
                 y = 0.98),
    annotations = list(text = str_wrap("Pro konzistentnost v průběhu let graf nezahrnuje zaměstnance ministerstev vnitra a zahraničních věcí, viz <b> Příloha 1: Data a metodologie. </b> Graf 4 s kapitolami MZV a MV je v příloze.",40),
                       align='left',
                       xref='paper',
                       yref='paper',
                       x=1.33,
                       ...
                       showarrow = FALSE),
    xaxis = list(title = ""),
    yaxis = list(title = "",range=(c(0,70)),gridcolor = "lightgrey"),
    barmode = "stack", legend = list(x = 100, y = 0.5)
  ) %>%
  config(modeBarButtonsToRemove = c("zoomIn2d", "zoomOut2d", "pan2d", "lasso2d", 
                                    "select2d", "autoScale2d"), 
...
graf_A4 <- vyvoj_bar %>%
  ...
  layout(
    plot_bgcolor='whitesmoke',
    title = list(font=list(color = "mediumblue"),text = "<b>Graf A4. Výdaje na platy státních úředníků 2003-2020 (miliardy Kč)</b>", y = 0.98),
    annotations = list(text = str_wrap("Pro konzistentnost v průběhu let graf nezahrnuje zaměstnance ministerstev vnitra a zahraničních věcí, viz <b> Příloha 1: Data a metodologie. </b>",40),
                       align='left',
                       xref='paper',
                       yref='paper',
                       x=1.33,
...
graf_A5 <- vyvoj_bar %>%
  ...
  layout(margin = list(t = 50),
    plot_bgcolor='whitesmoke',
    title = list(font=list(color = "mediumblue"),text = str_wrap("<b>Graf A5. Výdaje na platy státních úředníků 2003-2020 (miliardy Kč)</b>",75), y = 0.95),
    annotations = list(text = paste0("Reálné výdaje na platy.<br>",str_wrap("Pro konzistentnost v průběhu let graf nezahrnuje zaměstnance ministerstev vnitra a zahraničních věcí, viz <b> Příloha 1: Data a metodologie. </b>",40)),
                       align='left',
                       xref='paper',
                       yref='paper',
                       x=1.33,
...
aux2 <- dt %>%
  filter(kategorie_2014_cz %in% c("Státní úředníci celkem","Ministerstva", "Ostatní ústřední",
...
graf_A6 <- aux2 %>%
  plot_ly(
    ...
    mode = "line", line = list(width = 4),
    marker = list(size=0.5,symbol = "circle",line = list(width = 5,color="white")),
    text = ~ paste(
  ...
  layout(margin = list(t = 50),
    plot_bgcolor='whitesmoke',
    title = list(font=list(color = "mediumblue"),text = str_wrap("<b>Graf A6. Kumulovaná změna výdajů na platy státních úředníků 2003-2020</b>",70), y = 0.95),
    annotations = list(text =paste0("Reálné výdaje na platy.<br>",str_wrap("Pro konzistentnost v průběhu let graf nezahrnuje zaměstnance ministerstev vnitra a zahraničních věcí, viz <b> Příloha 1: Data a metodologie. </b>",40)),
                       align='left',
                       xref='paper',
                       yref='paper',
                       ...
                       showarrow = FALSE),
    xaxis = list(title = ""),
    yaxis = list(title = "", tickprefix = "+", ticksuffix = "%",gridcolor="darkgrey"), 
...
graf_5 <- dt %>%
  ...
  layout(
      annotations = list(text = str_wrap("Reálné průměrné mzdy v cenách roku 2020.",40),
                       align='left',
                       xref='paper',
                       yref='paper',
                       x=1.33,
    ...
    plot_bgcolor='whitesmoke',
    title = 
      list(font=list(color = "mediumblue"),text = "<b>Graf 5: Průměrné platy státních úředníků 2003-2020 (tisíce Kč)</b>", y =0.98),
    xaxis = list(title = ""),
    yaxis = list(title = "",gridcolor="darkgrey"), legend = list(x = 100, y = 0.5)
  ) %>%
  config(modeBarButtonsToRemove = c("zoomIn2d", "zoomOut2d", "pan2d", "lasso2d", 
                                    "select2d", "autoScale2d"), displaylogo = FALSE)
graf_5
...
graf_A7 <- dt %>%
  ...
  layout(
    plot_bgcolor='whitesmoke',
    title = list(font=list(color = "mediumblue"),text = "<b>Graf A7. Kumulovaná změna průměrných platů státních úředníků</b>", y = 0.98),
    annotations = list(text =paste0("Reálné výdaje na platy.<br>",str_wrap("Pro konzistentnost v průběhu let graf nezahrnuje zaměstnance ministerstev vnitra a zahraničních věcí, viz <b> Příloha 1: Data a metodologie. </b>",40)),
                       align='left',
                       xref='paper',
                       yref='paper',
...
graf_6<-dt %>%
  ...
  plot_ly(
    ...
    text = ~ paste(
      ...
      "Nejv\u011Bt\u0161\u00ED nárůst:", "<br>", max_change_kap, ": ", 
      max_change * 100, " %", "<br>",
      ifelse(min_change>0,"Nejmen\u0161\u00ED nárůst:","Nejv\u011Bt\u0161\u00ED pokles:"), "<br>", min_change_kap, ": ", 
      min_change * 100, " %", "<br>"
    ),
    hoverinfo = "text",
    legendgroup = ~kategorie_2014_cz
  ...
  layout(margin = list(t = 50),
    ...
    annotations = list(text = str_wrap("Pro ministerstva a ostatní ústřední orgány použité hodnoty průměrné mzdy v Praze. V ostatních případech je jako reference použitý průměrný plat v národním hospodářství. Hodnota 100% znamená, že průměrný plat v kategorii je stejný jako průměrný plat v národním hospodářství. Popisky “Největší a nejmenší nárust” zde vyjadřují procentuální změnu platu ve vztahu k průměrnému platu v ČR. Tedy pokud plat na Ministerstvu financí byl v jednom roce roven 115% pražského platu, a další rok vzrostl na 130%, největší nárůst by byl 15%.",40),
                       ...
                       showarrow = FALSE),
    xaxis = list(title = ""),
    yaxis = list(title = "", ticksuffix = "%",gridcolor="darkgrey"), legend = list(x = 100, y =0.8)
  ) %>%
  config(modeBarButtonsToRemove = c("zoomIn2d", "zoomOut2d", "pan2d", "lasso2d", 
                                    "select2d", "autoScale2d"), displaylogo = FALSE)
...
graf_A8 <- dt %>%
  ...
  layout(
    plot_bgcolor='whitesmoke',
    title = list(font=list(color = "mediumblue"),text = "<b>Graf A8. Celkové změny platů a počtu zaměstnanců v období 2004-2020</b>", y = 0.98),
    annotations = list(text = str_wrap("Pro konzistentnost v průběhu let graf nezahrnuje zaměstnance ministerstev vnitra a zahraničních věcí, viz <b> Příloha 1: Data a metodologie. </b>",40),
                       align='left',
                       xref='paper',
                       yref='paper',
    ...
    yaxis = list(title = "Zm\u011Bna po\u010Dtu zam\u011Bstnanc\u016F", 
                 ...
                 tickprefix = "+", 
                 ticksuffix = "%", showticksuffix = "all",gridcolor="darkgrey"), 
    legend = list(x = 100, y = 0.5)
  ) %>%
  add_markers() %>%
...
graf_A9 <- dt %>%
  ...
  group_map(~ plot_ly(
    ...
    layout(plot_bgcolor='whitesmoke',
           xaxis = list(title=""),
           yaxis = list(title = "", range = c(-30, 5), ticksuffix = "%",gridcolor="darkgrey"), 
           xaxis = list(tickangle = 90), legend = list(x = 100, y = 0.5)), keep = TRUE) %>%
  subplot(nrows = 5, shareX = TRUE, shareY = TRUE) %>%
  layout(title = list(font=list(color = "mediumblue"),text = "<b>Graf A9. Rozdíl mezi schváleným a skutečným počtem zaměstnanců</b>", 
...
graf_A10 <- dt %>%
  ...
  group_map(~ plot_ly(
    ...
    layout(plot_bgcolor='whitesmoke',
           xaxis = list(title=""),
           yaxis = list(title = "", range = c(0, 30), ticksuffix = "%",gridcolor="darkgrey"), 
           xaxis = list(tickangle = 90), legend = list(x = 100, y = 0.5)), 
  keep = TRUE) %>%
  subplot(nrows = 5, shareX = TRUE, shareY = T) %>%
  layout(title = list(font=list(color = "mediumblue"),text = "<b>Graf A10. Rozdíl v průměrných platech mezi schváleným rozpočtem a skutečností</b>",
                      y = 0.98), showlegend = FALSE)
...
graf_A11 <- dt %>%
  ...
  group_map(~ plot_ly(
    ...
    layout(plot_bgcolor='whitesmoke',
           xaxis = list(title=""),
           yaxis = list(title = "", range = c(-15, 1), ticksuffix = "%",gridcolor="darkgrey"), 
           legend = list(x = 100, y = 0.5), showlegend = F), keep = TRUE) %>%
  subplot(nrows = 2, shareX = TRUE, shareY = TRUE) %>%
  layout(title = list(font=list(color = "mediumblue"),text = "<b>Graf A11. Rozdíl mezi schváleným a skutečným počtem zaměstnanců</b>", 
                      y = 0.98))
```
...
graf_A12 <- dt %>%
  ...
  group_map(~ plot_ly(
    ...
    layout(plot_bgcolor='whitesmoke',
           xaxis = list(title=""),
           yaxis = list(title = "", range = c(0, 16), ticksuffix = "%",gridcolor="darkgrey"), 
           legend = list(x = 100, y = 0.5), showlegend = F), keep = TRUE) %>%
  subplot(nrows = 2, shareX = TRUE, shareY = TRUE) %>%
  layout(title = list(font=list(color = "mediumblue"),text = "<b>Graf A12. Rozdíl v průměrných platech mezi schváleným rozpočtem a skutečností </b>", 
                      y = 0.98))
```
...
graf_A13 <- dt %>%
  ...
  group_map(~ plot_ly(
    ...
    layout(plot_bgcolor='whitesmoke',
           yaxis = list(title = "", range = c(15, 75), ticksuffix = "%",gridcolor="darkgrey"),
           xaxis = list(title = "Pr\u016Fm\u011Brn\u00FD plat v roce 2003 (tisíce Kč)", range = c(15, 35)),
  ...
  layout(title = list(font=list(color = "mediumblue"),text = "<b>Graf A13. Nárůst průměrných platů od roku 2003</b>",
                      xaxis = list(title = ""), y = 0.98))
...
Statistické ročenky České republiky - za jednotlivé roky. (Zde například za rok 2021: <https://www.czso.cz/csu/czso/10-trh-prace-sdpe56comt>)
...
Pro další roky (1995, 1999-2019; ostatní roky nejsou podle všeho dostupné online) shrnujeme tyto informace v grafu A0 v příloze. Mezi lety 1995 (1646 tisíc) a 2012 (921,4 tisíc) došlo k výraznému poklesu počtu zaměstnanců veřejného sektoru a mezi roky 2012 a 2019 došlo k postupnému, mírnému nárůstu. 
...
Graf A0. Počet zaměstnanců veřejného sektoru celkem (v tisících přepočtených osob)^[Informace pro roky 1996-1998 a období před rokem 1995 nejsou dostupné online. 
...
```{r graf-a0, echo=FALSE, out.width="100%", warning = FALSE, message = FALSE}
pubsec <- read_csv(here::here("data-input", "ver-sektor-csu-rocenka.csv"))

graf_A0 <- plot_ly(pubsec, type = "scatter", mode = "lines+markers", line = list(width = 4),
    marker = list(size=0.5,symbol = "circle",line = list(width = 5,color="white")),
    hovertemplate = ~ paste(
          "<extra></extra>",
          "Rok: ", rok, "<br>",
          "Počet: ", pocet)) %>% 
          add_trace(x = ~rok, y = ~pocet, color = I("black"),
            name = "Počet zaměstnanců veřejného sektoru\n(tis. přepočtených osob)") %>% 
    layout(plot_bgcolor='whitesmoke',
           yaxis = list(title = NA, range = c(0, 1800),gridcolor="darkgrey"),
           xaxis = list(title = NA),
           legend = list(x = 100, y = 0.5), showlegend = F,
           title = list(font=list(color = "mediumblue"),text = "<b>Graf A0. Počet zaměstnanců veřejného sektoru (tis. přepočtených osob)</b>",
                        y = 0.98)) %>%
  config(modeBarButtonsToRemove = c("zoomIn2d", "zoomOut2d", "pan2d", "lasso2d",
                                    "select2d", "autoScale2d"), 
         displaylogo = FALSE)
...
### Časové řady zahrnující kapitoly MZV a MV
...
graf_B4<-dt %>%
  ...
  layout(
    plot_bgcolor='whitesmoke',
    title = list(font=list(color = "mediumblue"),text = "<b>Graf B4. Počet státních úředníků 2003-2020 (v tisících)</b>", y = 0.98),
