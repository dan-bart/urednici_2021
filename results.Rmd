...
graf_A3 <- bar_dt %>%
  ...
  group_map(~ plot_ly(
    ...
    layout(
      ...
      title = list(text = "Graf A3. Pr\u016Fm\u011Brn\u00E9 platy st\u00E1tn\u00EDch zam\u011Bstnanc\u016F (ve vztahu ke mzd\u011B v n\u00E1rodn\u00EDm hospod\u00E1\u0159stv\u00ED)", 
...
graf_5 <- dt %>%
  ...
  layout(
    plot_bgcolor='whitesmoke',
    title = 
      list(text = "Graf 5. Reálné průměrné hrubé měsíční platy (v tisících; ceny roku 2020)", y =0.98),
    xaxis = list(title = ""),
...
graf_A0 <- plot_ly(pubsec, type = "scatter", mode = "lines+markers", line = list(width = 4),
    ...
    layout(plot_bgcolor='whitesmoke',
           ...
           xaxis = list(title = NA),
           legend = list(x = 100, y = 0.5), showlegend = F,
           title = list(text = "Graf A0. Počet zaměstnanců veřejného sektoru (tis. přepočtených osob)",
                        y = 0.98)) %>%
  config(modeBarButtonsToRemove = c("zoomIn2d", "zoomOut2d", "pan2d", "lasso2d",
                                    "select2d", "autoScale2d"), 
         displaylogo = FALSE)
