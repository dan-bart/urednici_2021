...
graf_A1 <- tree_data %>%
  ...
  layout(title = list(font=list(color = cap_col),text = "<b>Graf A1. Výdaje na platy státních zaměstnanců dle regulace zaměstnanosti</b>", 
                      y = 0.98)) %>%
...
graf_1 <- tree_data %>%
  ...
  layout(title = list(font=list(color = cap_col),
                      text = "<b>Graf 1. Počet státních zaměstnanců dle regulace zaměstnanosti</b>", 
                      y = 0.98)) %>%
...
graf_2 <- bar_dt %>% group_by(kategorie_2014_cz)%>%
  ...
  layout(
    ...
    annotations = list(text = str_wrap("MŠMT zkresluje graf vzhledem k zahrnutí  učitelů v kategorii “Příspěvkové organizace”. Lze odflitrovat v legendě nebo v grafu.",40),
                       ...
                       borderwidth=1,
                       showarrow = FALSE),
    title = list(font=list(color = cap_col),text = "<b>Graf 2. Po\u010Det st\u00E1tn\u00EDch zam\u011Bstnanc\u016F dle rozpo\u010Dtov\u00FDch kapitol (tis\u00EDce)</b>", y = 0.97),
...
graf_A2 <- plot_ly(bar_dt,
  ...
  layout(
    ...
    title = list(font=list(color = cap_col),
                 text = "<b>Graf A2. Výdaje na platy státních zaměstnanců (miliardy K\u010D)</b>", y = 0.98),
...
graf_3 <- bar_dt %>% 
  ...
  layout(
    ...
    hovermode = "closest",
    title = list(font=list(color = cap_col),text = "<b>Graf 3. Pr\u016Fm\u011Brn\u00E9 platy st\u00E1tn\u00EDch zam\u011Bstnanc\u016F (tis\u00EDce K\u010D)</b>", y = 1.1),
...
graf_A3 <- bar_dt %>%
  ...
  group_map(~ plot_ly(
    ...
    layout(margin = list(t = 50),
      ...
      title = list(font=list(color = cap_col),text = str_wrap("<b>Graf A3. Pr\u016Fm\u011Brn\u00E9 platy st\u00E1tn\u00EDch zam\u011Bstnanc\u016F ve vztahu k pr\u016Fm\u011Brn\u00E9 mzd\u011B v n\u00E1rodn\u00EDm hospod\u00E1\u0159stv\u00ED</b>",70), 
...
graf_A9 <- dt %>%
  ...
  group_map(~ plot_ly(
    ...
    layout(plot_bgcolor='whitesmoke',
           xaxis = list(title=""),
           yaxis = list(title = "", range = c(-30, 5), ticksuffix = "%",gridcolor = grdclr), 
           xaxis = list(tickangle = 90), legend = list(x = 100, y = 0.5)), keep = TRUE) %>%
  subplot(nrows = 5, shareX = TRUE, shareY = TRUE) %>%
  layout(title = list(font=list(color = cap_col),text = "<b>Graf A9. Rozdíl mezi schváleným a skutečným počtem zaměstnanců</b>", 
...
graf_A10 <- dt %>%
  ...
  group_map(~ plot_ly(
    ...
    layout(plot_bgcolor='whitesmoke',
           xaxis = list(title=""),
           yaxis = list(title = "", range = c(0, 30), ticksuffix = "%",gridcolor = grdclr), 
           xaxis = list(tickangle = 90), legend = list(x = 100, y = 0.5)), 
  keep = TRUE) %>%
  subplot(nrows = 5, shareX = TRUE, shareY = T) %>%
  layout(title = list(font=list(color = cap_col),text = "<b>Graf A10. Rozdíl v průměrných platech mezi schváleným rozpočtem a skutečností</b>",
                      y = 0.98), showlegend = FALSE, margin = mrg2) |> 
  config(modeBarButtonsToRemove = btnrm, displaylogo = FALSE)
...
graf_A11 <- dt %>%
  ...
  layout(title = list(font=list(color = cap_col),text = "<b>Graf A11. Rozdíl mezi schváleným a skutečným počtem zaměstnanců</b>", 
                      y = 0.98), margin = mrg2) |> 
  config(modeBarButtonsToRemove = btnrm, displaylogo = FALSE)
```
